...
export async function POST(request) {
  try {
    const { title, description, assigned_to, priority = 'medium' } = await request.json();
    const { data } = await supabaseAdmin.from('tasks').insert({ title, description, assigned_to, priority, status: 'backlog' }).select();
...
export async function PUT(request) {
  try {
    const { id, status, result, assigned_to } = await request.json();
