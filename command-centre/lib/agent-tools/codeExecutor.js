 ...
 */
function listProjectFiles(directory = 'src') {
 ...
 */
function rollbackFile(filePath, timestamp) {
  const fullPath = path.resolve(PROJECT_ROOT, filePath);
  const backupPath = path.join(BACKUP_DIR, timestamp, filePath);
...
function getProjectStructure() {
  try {
    ...
    // Try pages if it exists
    try { structure.pages = listProjectFiles('pages'); } catch(e) {}
    try { structure.styles = listProjectFiles('styles'); } catch(e) {}
    try { structure.public = listProjectFiles('public'); } catch(e) {}
    
    return JSON.stringify(structure, null, 2);
  } catch(e) {
    // If listing fails, return basic info
    return JSON.stringify({ error: e.message, app: [], components: [], lib: [] });
  }
}
