...
// Helper to fetch from Command Centre API
async function fetchFromAPI(endpoint) {
  ...
  try {
    ...
    console.log(`[fetchFromAPI] Error: ${err.message}`);
    return null;
  }
}
  ...
  return {
    ...
    async log(message, logType = 'info') {
      try {
        ...
        if (!res.ok) {
          console.log(`[Client.log] Error: ${res.status}`);
          return null;
        }
        return await res.json();
      } catch (err) {
        console.log(`[Client.log] Exception: ${err.message}`);
        return null;
      }
    },
    ...
    async updateTaskStatus(taskId, status, result = null) {
      ...
      try {
        ...
        if (!res.ok) {
          console.log(`[Client.updateTaskStatus] Error: ${res.status}`);
          return null;
        }
        return await res.json();
      } catch (err) {
        console.log(`[Client.updateTaskStatus] Exception: ${err.message}`);
        return null;
      }
    },
...
async function executeWithMinimax(prompt, client = null, agent = null) {
  ...
  // GroupId must be in URL as query param
...
IMPORTANT: Web search results are ALREADY PROVIDED in the context below. Do NOT attempt to call any tools. Use ONLY the information provided in the context.` },
  ...
  try {
    ...
    console.error(`[Minimax] JSON parse error: ${parseErr.message}`);
    console.error(`[Minimax] Response text: ${text.substring(0, 200)}`);
    throw new Error(`Minimax JSON parse error: ${parseErr.message}`);
  }
...
Only suggest subtasks if is_complex is true. Maximum 2 subtasks.`;
      ...
      // GroupId must be in URL as query param
      const groupId2 = MINIMAX_GROUP_ID || '';
      const apiUrl2 = `https://api.minimax.io/v1/text/chatcompletion_v2${groupId2 ? `?GroupId=${groupId2}` : ''}`;
      
      const complexityRes = await fetch(apiUrl2, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${MINIMAX_API_KEY}`
